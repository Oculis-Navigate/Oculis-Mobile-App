...
class AppDelegate: UIResponder, UIApplicationDelegate {
  ...
  func application(
    ...
    // Add more thorough debug for image availability
    let imageNames = ["lighthouse", "lighthouse.png"]
    for name in imageNames {
        if let image = UIImage(named: name) {
            print("✅ Image '\(name)' is available with size: \(image.size)")
        } else {
            print("❌ Image '\(name)' not found")
        }
    }

    // Add this to your existing debug code
    print("Launch screen storyboard path: \(Bundle.main.path(forResource: "LaunchScreen", ofType: "storyboardc") ?? "not found")")

    // Force preload the image
    if let lighthouseImage = UIImage(named: "lighthouse.png") {
        let imageView = UIImageView(image: lighthouseImage)
        print("Successfully created imageView with lighthouse image")
    }
